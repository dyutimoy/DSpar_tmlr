...
class QCustomGATConv(CustomGATConv):
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(self.in_channels, int):
            ...
            self.lin_l = Linear(self.in_channels[0], self.heads * self.out_channels, False)
            self.lin_r = Linear(self.in_channels[1], self.heads * self.out_channels, False)

        if self.residual:
            self.res_fc = Linear(self.in_channels, self.heads * self.out_channels, bias=False)
        else:
            self.register_buffer("res_fc", None)
        self.dropout_module = torch.nn.Dropout(p=self.dropout)
